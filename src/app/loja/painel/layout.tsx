...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-zinc-950">
      <aside className="relative hidden min-h-screen w-64 shrink-0 border-r border-zinc-800 bg-zinc-900/50 lg:block">
        <div className="flex h-16 items-center gap-2 border-b border-zinc-800 px-6">
          <Car className="h-6 w-6 text-amber-400" />
          <span className="font-display font-bold text-white">Agury</span>
        </div>
        ...
        <div className="absolute bottom-4 left-4 right-4 lg:w-56">
          <form action="/api/auth/logout" method="POST">
            <button type="submit" className="flex w-full items-center gap-2 rounded-xl px-4 py-2 text-sm text-zinc-500 hover:text-red-400">
      ...
      <main className="flex-1 overflow-auto">
        <div className="border-b border-zinc-800 px-6 py-4 lg:hidden">
          <p className="font-display font-bold text-white">{store?.name || "Painel"}</p>
        </div>
